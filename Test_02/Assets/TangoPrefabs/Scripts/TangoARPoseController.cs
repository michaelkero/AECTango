...
public class TangoARPoseController : MonoBehaviour, ITangoLifecycle
{
    ...
    private void _UpdateTransformation(double timestamp)
    {
        ...
        GameObject obj = GameObject.Find("Tango Delta Camera");
        Transform trans = obj.transform;
        Vector3 vect = trans.localPosition;

        GameObject textObj = GameObject.Find("TextBox");
        Text text = textObj.GetComponent<Text>();
        //text.text = vect.x +" , " + vect.y + " , " + vect.z;
        //text.text = transform.position.x + " , " + transform.position.y + " , " + transform.position.z;

        double x = Math.Round(transform.position.x, 4);
        double y = Math.Round(transform.position.y, 4);
